			...
			<div class="review-write-box" >
				...
				<form id="reviewForm" method="post" enctype="multipart/form-data">
					...
					<div class="compact-rating-area">
						...
						<span class="compact-rating-text" id="ratingText"> {{ rstore.r_score === 0 ? '별점을 선택해주세요' : rstore.r_score + '점' }} </span>
					</div>
					<input type="hidden" name="rating" id="ratingValue" v-model="rstore.r_score">
					<!-- 리뷰 내용 -->
					<textarea class="compact-textarea" name="content" id="reviewContent" placeholder="상품에 대한 솔직한 리뷰를 작성해주세요." maxlength="500" required v-model="rstore.r_content" ref="cref"></textarea>
				...
				<div class="container">
											...
											<li v-for="(vo,index) in rstore.list" :key="index"
												...
												<div class="comment-body">
													<div class="comment-author">
														<img :src="vo.profile_img ? vo.profile_img : '/images/noimage.png'" class="img-circle" style="width: 80px; height: 80px;">
													</div>
													<div class="comment-info">
														<div class="comment-header rating-stars">
															<h3 class="comments-title" :title="vo.name">
																{{vo.name}}&nbsp; <span class="rating-star" v-if="vo.r_score != null"> 
																<i class="fa fa-star" v-for="n in rstore.starCount(vo.r_score).full" :key="'f'+n"></i>
																<i class="fa fa-star-half-o" v-if="rstore.starCount(vo.r_score).half"></i>
															    <i class="fa fa-star-o" v-for="n in rstore.starCount(vo.r_score).empty" :key="'e'+n"></i>
																</span>
															</h3>
					...
					<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
						<div class="st-pagination">
							<ul class="pagination">
								<li v-if="rstore.startPage>1">
									<a href="#" @click="rstore.pageChange(rstore.startPage-1)">이전</a>
								</li>
								<li :class="i==rstore.curpage?'active':''" v-for="i in rstore.range">
									<a href="#" @click="rstore.pageChange(i)">{{i}}</a>
								</li>
								<li v-if="rstore.endPage<rstore.totalpage">
									<a href="#" @click="rstore.pageChange(rstore.endPage+1)">다음</a>
								</li>
							</ul>
						</div>
					</div>
