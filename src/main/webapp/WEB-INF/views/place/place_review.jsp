...
<body>
	<div
		style="border: 1px solid #e3dfdc; border-top: none; border-radius: 0 0 5px 5px; padding: 20px; margin-bottom: 50px;">
					...
					<div class="compact-rating-area">
						<div class="compact-star-rating" id="starRating">
							<span class="star-wrapper" v-for="n in 5" :key="n"
								...
								<i class="fa"
								:class="n <= Math.floor(rstore.r_score) ? 'fa-star' : (n === Math.ceil(rstore.r_score) && rstore.r_score % 1 !== 0 ? 'fa-star-half-o' : 'fa-star-o')"></i>
							</span>
						</div>
						<span class="compact-rating-text" id="ratingText"> {{
							rstore.r_score === 0 ? '별점을 선택해주세요' : rstore.r_score + '점' }} </span>
					</div>
					<input type="hidden" name="rating" id="ratingValue"
						v-model="rstore.r_score">
					<!-- 리뷰 내용 -->
					<textarea class="compact-textarea" name="content"
						id="reviewContent" placeholder="상품에 대한 솔직한 리뷰를 작성해주세요."
						maxlength="500" required v-model="rstore.r_content" ref="cref"></textarea>
					<div style="text-align: right;">
						<button type="button" class="btn-submit-review"
							@click="rstore.reviewInsert()">
		...
		<div class="rating-summary">
			<div class="rating-score">
				<div class="rating-number">{{rstore.avg_score ?
					rstore.avg_score: '-'}}</div>
		...
		<div style="display: inline; width: 1155px;" id="reviewApp">
			<div
				style="display: flex; flex-wrap: wrap; gap: 12px; width: 100%; margin-bottom: 10px; margin-left: 32px; padding-top: 40px;">
				<div class="container">
					<div class="row">
								...
								<div class="row">
									<div class=" col-lg-12 col-md-12">
										<ul class="comment-list">
											<li v-if="rstore.list.length===0" class="text-center"><h3>리뷰가
													없습니다.</h3></li>
											<li v-for="(vo,index) in rstore.list" :key="index"
												v-if="rstore.list.length>0">
												<div class="comment-body">
													<div class="comment-author">
														<img
															:src="vo.profile_img ? vo.profile_img : '/images/noimage.png'"
															class="img-circle" style="width: 80px; height: 80px;">
													</div>
													<div class="comment-info">
														<div class="comment-header rating-stars">
															<h3 class="comments-title" :title="vo.name">
																{{vo.name}}&nbsp; <span class="rating-star"
																	v-if="vo.r_score != null"> <i class="fa fa-star"
																	v-for="n in rstore.starCount(vo.r_score).full"
																	:key="'f'+n"></i> <i class="fa fa-star-half-o"
																	v-if="rstore.starCount(vo.r_score).half"></i> <i
																	class="fa fa-star-o"
															...
															<sec:authorize access="isAuthenticated()">
																<div class="text-left" style="margin-right: auto;">
																	<a href="/mypage/my_review" v-if="sessionId===id">수정
																		/ 삭제</a>
																</div>
															</sec:authorize>
															<div class="meta">
																<span v-if="vo.uday"> 수정일 : {{ vo.uday }} </span> <span
																	v-else>작성일 : {{ vo.dbday }} </span>
															</div>
														</div>
					...
					<div class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
						<div class="st-pagination">
							<ul class="pagination">
								<li v-if="rstore.startPage>1"><a href="#"
									@click="rstore.pageChange(rstore.startPage-1)">이전</a></li>
								<li :class="i==rstore.curpage?'active':''"
									v-for="i in rstore.range"><a href="#"
									@click="rstore.pageChange(i)">{{i}}</a></li>
								<li v-if="rstore.endPage<rstore.totalpage"><a href="#"
									@click="rstore.pageChange(rstore.endPage+1)">다음</a></li>
							</ul>
						</div>
					</div>
				</div>
