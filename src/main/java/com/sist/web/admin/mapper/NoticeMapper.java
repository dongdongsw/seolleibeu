...
public interface NoticeMapper {
	@Select("SELECT n_id, n_title, n_created_at, hit FROM notice "
			   +"ORDER BY n_id DESC "
	...
	@Select("SELECT n_title, n_created_at, hit, n_content "
	...
	@Update("UPDATE notice SET "
		   +"hit=hit+1 "
		   +"WHERE u_id=#{u_id}")
	public void noticeHitIncrement(int u_id);
	...
	@Delete("DELETE FROM notice WHERE n_id=#{u_id}")
